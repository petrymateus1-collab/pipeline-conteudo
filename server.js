...
async function classificarFases(transcricao, durTotal) {
  ...
  try {
    ...
    const prompt = `Voce e um especialista em copywriting de videos curtos para redes sociais.
Analise a transcricao abaixo e identifique os timestamps de inicio e fim de cada fase do roteiro.
As fases sao: hook, participacao, body, reframe, cta.
Duracao total: ${durTotal.toFixed(1)}s

Transcricao:
${texto}

Responda APENAS com JSON valido neste formato exato, sem explicacoes, sem markdown, sem backticks:
...
async function montarVideo(videoPath, workDir, assets) {
  ...
  run('ffmpeg -y -i "' + vidCtaPath + '" -i "' + musicaFinal + '" -filter_complex "[1:a]apad,atrim=0:' + durTotal + ',asetpts=PTS-STARTPTS,afade=t=out:st=' + (durTotal - 2).toFixed(2) + ':d=2[mus];[0:a]volume=2.5[orig];[mus][orig]amix=inputs=2:duration=first:dropout_transition=0[afinal]" -map "0:v" -map "[afinal]" -c:v copy -c:a aac -b:a 128k -ar 44100 -t ' + durTotal + ' "' + outputPath + '"');
...
app.post("/processar", upload.single("video"), async (req, res) => {
  ...
  try {
    ...
    if (req.file) {
      ...
      const bodyPath = path.join(workDir, "input_body.mp4");
      log("Baixando body: " + req.body.video_url);
      await downloadFile(req.body.video_url, bodyPath);

      if (req.body.hook_url) {
        const hookPath = path.join(workDir, "input_hook.mp4");
        log("Baixando hook: " + req.body.hook_url);
        await downloadFile(req.body.hook_url, hookPath);
